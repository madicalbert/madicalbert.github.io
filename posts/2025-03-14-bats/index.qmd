---
title: "White-Nose Syndrome and Pesticide Use"
description: "Investigating the connection between White-nose syndrome in bats and increases in pesticide use in agriculture in the US."
author:
  - name: Madi Calbert & Steven Mitchell
    url: https://madicalbert.github.io/
    affiliation: ESM 217 Planetary Health Course - Master of Environmental Science & Management Program @ The Bren School (UCSB)
    affiliation-url: https://bren.ucsb.edu/masters-programs/master-environmental-science-and-management 
date: March 14, 2025
categories: [Data Science]
citation: 
  url: https://madicalbert.github.io/posts/2025-03-14-bats/ 
...
![A bat suffering from White-nose syndrome, a disease caused by the fungus *Pseudomycans destructans* (commonly abbreviated as "Pd"). *Photo credit: Google Creative Commons*](preview-image.png)
...
us <- read_sf(here("posts", "2025-03-14-bats", "data", "tl_2024_state", "tl_2024_us_state.shp")) %>% 
...
counties <- read_sf(here("posts", "2025-03-14-bats", "data", "tl_2023_us_county", "tl_2023_us_county.shp")) %>% 
  mutate(county = NAME)
```
...
wns <- read_sf(here("posts", "2025-03-14-bats", "data", "wns_county_status", "wns_county_status.csv"))
...
tmap_save(map_wns_counties, here("posts", "2025-03-14-bats", "outputs", "map_wns_counties.png"))
```
...
```{r pesticides}
pesticides_2013_2017 <- read_csv(here("posts", "2025-03-14-bats", "data", "2013_2017_pesticides", "EPest_county_estimates_2013_2017_v2.csv")) %>% 
...
pesticides_2018 <- read_csv(here("posts", "2025-03-14-bats", "data", "2018_pesticides", "EPest_county_estimates_2018.csv")) %>% 
  mutate(COUNTYFP = as.character(COUNTY_FIPS_CODE)) %>% 
  mutate(concentration = ifelse(is.na(EPEST_HIGH_KG), 
                                EPEST_LOW_KG, EPEST_HIGH_KG)) %>% 
  group_by(COUNTYFP, YEAR) %>% 
...
pesticides_2019 <- read_csv(here("posts", "2025-03-14-bats", "data", "2019_pesticides", "EPest_county_estimates_2019.csv")) %>% 
  mutate(COUNTYFP = as.character(COUNTY_FIPS_CODE)) %>% 
  mutate(concentration = ifelse(is.na(EPEST_HIGH_KG), 
                                EPEST_LOW_KG, EPEST_HIGH_KG)) %>% 
  group_by(COUNTYFP, YEAR) %>% 
...
tmap_save(map_pesticides_counties, here("posts", "2025-03-14-bats", "outputs", "map_pesticides_counties.png"))
```
...
write_sf(wns_pesticides, here("posts", "2025-03-14-bats", "data", "wns_pesticides.shp"))
...
ggsave(here("posts", "2025-03-14-bats", "outputs", "chart_wns_pesticides.png"), chart_wns_pesticides,
       width = 6, height = 3)
```
...
ggsave(here("posts", "2025-03-14-bats", "outputs", "map_bivar.png"), map_bivar, dpi = 400, width = 8.48, height = 5.12)
```
...
```{r}
# Infant Mortality
infant_mortality <- read_csv(here("posts", "2025-03-14-bats", "data", "infant_mortality", "infant_mortality.csv")) %>%   clean_names() %>% 
...
map_wns_infmort <- tm_shape(us, bbox = bbox)+
  ...
  tm_layout(bg.color = "#f6f9f3",
            outer.bg.color = "#f6f9f3",
            frame.lwd = 0)
tmap_save(map_wns_infmort, here("posts", "2025-03-14-bats", "outputs", "map_wns_infmort.png"))
```
